...
const ActressPage = () => {
    ...
    const handleUserClick = (user) => {
        navigate('/user-profile', {
            state: {
                sub_cat_id: ACTRESS_SUBCATEGORY_ID,
                user_id: user.userId
            }
        });
    };
    ...
    const ACTRESS_SUBCATEGORY_ID = '87'; // Subcategory ID for Actress
    ...
    const [pageInfo, setPageInfo] = useState({
        name: 'Actress',
        description: 'Female lead / Supporting Performers'
    });
    ...
    // Fetch actresses based on applied filters
    useEffect(() => {
        const fetchActresses = async () => {
            try {
                ...
                let apiUrl = `${BASE_URL}/api/getUserListBasedOnSubCat/${ACTRESS_SUBCATEGORY_ID}`;
                ...
                console.log('Fetching actresses from:', apiUrl);
                ...
                if (!response.ok) {
                    throw new Error(`Failed to fetch actresses: ${response.status}`);
                }

                const result = await response.json();
                console.log('Actresses List API Response:', result);
    ...
    const handleClearFilters = () => {
        ...
        // Clear applied filters (this will trigger API to fetch all actresses)
        setAppliedCountry('');
        setAppliedState('');
        setAppliedCity('');
    };
